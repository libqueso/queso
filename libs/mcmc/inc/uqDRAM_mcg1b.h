  ...
  bool     applyMahalanobisInvert)
{
  ...
  for (unsigned int chainId = 0; chainId < m_chainSizes.size(); ++chainId) {
    ...
    if (m_chainType == UQ_MCMC_WHITE_NOISE_CHAIN_TYPE) {
      ...
      //****************************************************
      if (chainId > 0) {
        m_chain1.getPositionValues(m_chain1.sequenceSize()-1,valuesOf1stPosition);
  ...
  const std::string& chainName)
{
  ...
  meanVec.cwSet(0.);
  stdDevVec.cwSet(1.);
  m_chain1.setGaussian(m_env.rng(),meanVec,stdDevVec);
#else
  ...
  std::ofstream*     passedOfs)
{
  ...
  m_chain1.setPositionValues(0,currentPosition.paramValues());
  ...
  for (unsigned int positionId = 1; positionId < m_chain1.sequenceSize(); ++positionId) {
    ...
    //****************************************************
    if (accept) {
      m_chain1.setPositionValues(positionId,currentCandidate.paramValues());
      ...
      m_chain1.setPositionValues(positionId,currentPosition.paramValues());
      if (m_chainGenerateExtra) {
        if (m_likelihoodObjComputesMisfits) {
    ...
    if ((m_initialNonAdaptInterval > 0) &&
      ...
      if (subChain1.sequenceSize() > 0) {
        ...
        for (unsigned int i = 0; i < subChain1.sequenceSize(); ++i) {
          m_chain1.getPositionValues(idOfFirstPositionInSubChain+i,transporterVec);
          subChain1.setPositionValues(i,transporterVec);
        }
...
uqDRAM_MarkovChainGeneratorClass<V,M>::updateCovMatrix1(
  const uqChainBaseClass<V>& subChain1,
