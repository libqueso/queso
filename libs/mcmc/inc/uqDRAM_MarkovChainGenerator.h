...
#define UQ_MCMC_PSD_DISPLAY_ODV                  0
...
private:
  ...
  //                                double b,
  //                                M&     mat);
  void   computeStatistics       (const std::vector<const V*>&      chain1,
                                  const uqArrayOfSequencesClass<V>& chain2,
                                  unsigned int                      chainId,
                                  std::ofstream*                    passedOfs);
  int    writeChain              (const std::vector<const V*>&      chain1,
                                  const uqArrayOfSequencesClass<V>& chain2,
                                  unsigned int                      chainId,
                                  std::ofstream&                    ofs,
                                  const M*                          mahalanobisMatrix = NULL,
  ...
  std::string m_option_psd_display;
  ...
  bool                       m_psdCompute;
  std::vector<unsigned int>  m_psdNumBlocks;
  ...
  unsigned int               m_uncorrInitialPos;         // set during run time
  unsigned int               m_uncorrSpacing;            // set during run time
  V*                         m_statsMinPositions;        // set during run time
  V*                         m_statsMaxPositions;        // set during run time

  bool                       m_histCompute;
  unsigned int               m_histNumberOfInternalBins;
  std::vector<V*>            m_histCentersForAllBins;    // set during run time
  std::vector<V*>            m_histBinsForAllParams;     // set during run time

  bool                       m_kdeCompute;
  unsigned int               m_kdeNumberOfEvalPositions;
  std::vector<V*>            m_kdeEvalPositions;         // set during run time
  V*                         m_kdeScales;                // set during run time 
  std::vector<V*>            m_kdeGaussianDensityValues; // set during run time
...
uqDRAM_MarkovChainGeneratorClass<V,M>::uqDRAM_MarkovChainGeneratorClass(
  ...
  m_numberOfChains               (UQ_MCMC_CHAIN_NUMBER_ODV),
  m_sizesOfChains                (1,(unsigned int) strtod(UQ_MCMC_CHAIN_SIZES_ODV,NULL)),
  ...
  m_measureRunTimes              (UQ_MCMC_CHAIN_MEASURE_RUN_TIMES_ODV),
  ...
  m_corrDisplay                  (false),
  m_corrWrite                    (false),
  m_uncorrInitialPos             (0),
  m_uncorrSpacing                (0),
  m_statsMinPositions            (m_paramSpace.newVector()),
  m_statsMaxPositions            (m_paramSpace.newVector()),
  m_histCompute                  (UQ_MCMC_HIST_COMPUTE_ODV),
  m_histNumberOfInternalBins     (0),
  m_histCentersForAllBins        (0),//,NULL),
  m_histBinsForAllParams         (0),//,NULL),
  m_kdeCompute                   (UQ_MCMC_KDE_COMPUTE_ODV),
  m_kdeNumberOfEvalPositions     (0),
  m_kdeEvalPositions             (0),//,NULL),
  m_kdeScales                    (m_paramSpace.newVector()),
  m_kdeGaussianDensityValues     (0),//,NULL),
  ...
  m_lastAdaptedCovMatrix         (NULL)
{
  ...
  m_option_psd_display                = m_prefix + "MCMC_psd_display";
  ...
  m_psdNumBlocks.resize(1);
  m_psdNumBlocks[0] = 8;

  m_psdHopSizeRatio = .50;
...
uqDRAM_MarkovChainGeneratorClass<V,M>::~uqDRAM_MarkovChainGeneratorClass()
{
  ...
  for (unsigned int i = 0; i < m_kdeGaussianDensityValues.size(); ++i) {
    if (m_kdeGaussianDensityValues[i] != NULL) delete m_kdeGaussianDensityValues[i];
  }
  if (m_kdeScales != NULL) delete m_kdeScales;
  for (unsigned int i = 0; i < m_kdeEvalPositions.size(); ++i) {
    if (m_kdeEvalPositions[i] != NULL) delete m_kdeEvalPositions[i];
  }
  for (unsigned int i = 0; i < m_histBinsForAllParams.size(); ++i) {
    if (m_histBinsForAllParams[i] != NULL) delete m_histBinsForAllParams[i];
  }
  for (unsigned int i = 0; i < m_histCentersForAllBins.size(); ++i) {
    if (m_histCentersForAllBins[i] != NULL) delete m_histCentersForAllBins[i];
  }
  if (m_statsMaxPositions != NULL) delete m_statsMaxPositions;
  if (m_statsMinPositions != NULL) delete m_statsMinPositions;

  m_psdNumBlocks.clear();
  ...
  po::options_description& optionsDesc) const
{
  optionsDesc.add_options()
    ...
    (m_option_psd_compute.c_str(),                po::value<bool        >()->default_value(UQ_MCMC_PSD_COMPUTE_ODV                 ), "compute power spectral densities"                       )
#if 0
    (m_option_psd_numBlocks.c_str(),              po::value<            >()->default_value(), "")
    (m_option_psd_hopSizeRatio.c_str(),           po::value<            >()->default_value(), "")
#endif
  ...
  po::options_description& optionsDesc)
{
  ...
  if (m_env.allOptionsMap().count(m_option_chain_number.c_str())) {
    m_numberOfChains = m_env.allOptionsMap()[m_option_chain_number.c_str()].as<unsigned int>();
  }
  ...
  if (m_env.allOptionsMap().count(m_option_chain_sizes.c_str())) {
    ...
    if ((inputDoubles.size() == 1               ) && 
        (inputDoubles.size() != m_numberOfChains)) {
      inputDoubles.resize(m_numberOfChains,inputDoubles[0]);
    }
  ...
  if (m_env.allOptionsMap().count(m_option_chain_measureRunTimes.c_str())) {
    m_measureRunTimes = m_env.allOptionsMap()[m_option_chain_measureRunTimes.c_str()].as<bool>();
  }
  ...
  if (m_env.allOptionsMap().count(m_option_chain_outputFileNames.c_str())) {
    ...
    if ((m_namesOfOutputFiles.size() == 1                     ) && 
        (m_namesOfOutputFiles.size() != m_sizesOfChains.size())) {
      m_namesOfOutputFiles.resize(m_sizesOfChains.size(),m_namesOfOutputFiles[0]);
    }
  ...
  if (m_env.allOptionsMap().count(m_option_chain_use2.c_str())) {
    m_useChain2 = m_env.allOptionsMap()[m_option_chain_use2.c_str()].as<bool>();
  }
  ...
  std::ofstream*                    passedOfs)
{
  ...
  if (m_useChain2) {
    ...
    intChainSequenceSize    =          chain1.size();
    doubleChainSequenceSize = (double) chain1.size();
  }
  ...
  if ((m_psdCompute                      ) &&
    ...
    uq2dArrayOfStuff<V> _2dArrayOfPSDAtZero(initialPosForStatistics.size(),m_psdNumBlocks.size());
    ...
    if (m_useChain2) {
      chain2.psdAtZero(initialPosForStatistics,
                       m_psdNumBlocks,
                       m_psdHopSizeRatio,
                       _2dArrayOfPSDAtZero);
    }
    else {
      uqVectorSequencePSDAtZero(chain1,
                                initialPosForStatistics,
                                m_psdNumBlocks,
                                m_psdHopSizeRatio,
                                _2dArrayOfPSDAtZero);
    ...
    if ((m_psdDisplay) && (m_env.rank() == 0)) {
      for (unsigned int initialPosId = 0; initialPosId < initialPosForStatistics.size(); initialPosId++) {
        ...
        sprintf(line,"%s",
                "Parameter");
        std::cout << line;
        for (unsigned int numBlocksId = 0; numBlocksId < m_psdNumBlocks.size(); numBlocksId++) {
          sprintf(line,"%10s%3d",
                  " ",
                  m_psdNumBlocks[numBlocksId]);
          std::cout << line;
        }
    ...
    // Write PSD
    if (m_psdWrite && passedOfs) {
      std::ofstream& ofs = *passedOfs;
      ofs << "queso_" << m_prefix << chainId << "_" << "psdAtZero_numBlocks = zeros(" << 1
          << ","                                                                      << m_psdNumBlocks.size()
          << ");"
          << std::endl;
      for (unsigned int numBlocksId = 0; numBlocksId < m_psdNumBlocks.size(); numBlocksId++) {
        ofs << "queso_" << m_prefix << chainId << "_" << "psdAtZero_numBlocks(" << 1
            << ","                                                              << numBlocksId+1
            << ") = "                                                           << m_psdNumBlocks[numBlocksId]
            << ";"
            << std::endl;
      ...
      for (unsigned int initialPosId = 0; initialPosId < initialPosForStatistics.size(); initialPosId++) {
        ofs << "queso_" << m_prefix << chainId << "_" << "psdAtZero_initPos" << initialPosForStatistics[initialPosId] << " = zeros(" << m_paramSpace.dim()
            << ","      << m_psdNumBlocks.size()
            << ");"
            << std::endl;
        for (unsigned int i = 0; i < m_paramSpace.dim(); ++i) {
          for (unsigned int numBlocksId = 0; numBlocksId < m_psdNumBlocks.size(); numBlocksId++) {
            ofs << "queso_" << m_prefix << chainId << "_" << "psdAtZero_initPos" << initialPosForStatistics[initialPosId] << "(" << i+1
                << ","                                                                                                           << numBlocksId+1
  ...
  if ((m_corrComputeViaDef               ) &&
    ...
    tmpRunTime += uqMiscGetEllapsedSeconds(&timevalTmp);
    if (m_env.rank() == 0) {
      std::cout << "Chain autocorrelation took " << tmpRunTime
                << " seconds"
                << std::endl;
    }
    ...
    if (m_corrWrite && passedOfs) {
      std::ofstream& ofs = *passedOfs;
      ofs << "queso_" << m_prefix << chainId << "_" << "corr_lags = zeros(" << 1
          << ","                                                            << lagsForCorrs.size()
          << ");"
          << std::endl;
      for (unsigned int lagId = 0; lagId < lagsForCorrs.size(); lagId++) {
        ofs << "queso_" << m_prefix << chainId << "_" << "corr_lags(" << 1
            << ","                                                    << lagId+1
            << ") = "                                                 << lagsForCorrs[lagId]
            << ";"
            << std::endl;
      ...
      for (unsigned int initialPosId = 0; initialPosId < initialPosForStatistics.size(); initialPosId++) {
        ofs << "queso_" << m_prefix << chainId << "_" << "corr_initPos" << initialPosForStatistics[initialPosId] << " = zeros(" << m_paramSpace.dim()
            << ","      << lagsForCorrs.size()
            << ");"
            << std::endl;
        for (unsigned int i = 0; i < m_paramSpace.dim(); ++i) {
          for (unsigned int lagId = 0; lagId < lagsForCorrs.size(); lagId++) {
            ofs << "queso_" << m_prefix << chainId << "_" << "corr_initPos" << initialPosForStatistics[initialPosId] << "(" << i+1
                << ","                                                                                                      << lagId+1
                << ") = "                                                                                                   << _2dArrayOfAutoCorrs(initialPosId,lagId)[i]
                << ";"
                << std::endl;
  ...
  //****************************************************
  // Compute data related to uncorrelation
  //****************************************************
  m_uncorrInitialPos = 20000;
  m_uncorrSpacing    = 50;
  ...
  if ((m_histCompute                 ) &&
    ...
    for (unsigned int i = 0; i < m_statsMaxPositions->size(); ++i) {
      (*m_statsMaxPositions)[i] *= (1. + 1.e-15);
    }

    m_histCentersForAllBins.resize(m_histNumberOfInternalBins+2,NULL);
  ...
  if ((m_kdeCompute                  ) &&
    ...
    m_kdeEvalPositions.resize(m_kdeNumberOfEvalPositions,NULL);
    uqMiscComputePositionsBetweenMinMax(*m_statsMinPositions,
                                        *m_statsMaxPositions,
                                        m_kdeEvalPositions);
    ...
    if (m_env.rank() == 0) {
      std::cout << "\nComputed min values, max values and inter quantile ranges, for subchain beggining at position " << uncorrInitialPos
                  << std::endl;
      ...
      for (unsigned int i = 0; i < m_paramSpace.dim(); ++i) {
        ...
        sprintf(line,"%2s%11.4e%2s%11.4e%2s%11.4e",
                " ",
                (*m_statsMinPositions)[i],
                " ",
                (*m_statsMaxPositions)[i],
                " ",
                iqrs[i]);
        std::cout << line;
      }
  ...
  bool                              applyMahalanobisInvert) const
{
  ...
  if (m_useChain2) {
    ...
    chainSize = chain1.size();
    vectorSize = chain1[0]->size();
  }
  ...
  if (m_chainWrite) {
    ...
    if (m_generateExtraChains) {
      if (m_likelihoodObjComputesMisfits) {
        // Write m_misfitChain
        ofs << "queso_" << m_prefix << chainId << "_" << "misfitChain = zeros(" << m_misfitChain.size()
            << ","                                                              << m_misfitChain[0]->size()
            << ");"
            << std::endl;
        ofs << "queso_" << m_prefix << chainId << "_" << "misfitChain = [";
        for (unsigned int i = 0; i < m_misfitChain.size(); ++i) {
          ofs << *(m_misfitChain[i])
              << std::endl;
        }
        ...
        ofs << "queso_" << m_prefix << chainId << "_" << "misfitVarianceChain = zeros(" << m_misfitVarianceChain.size()
            << ","                                                                      << m_misfitVarianceChain[0]->size()
            << ");"
            << std::endl;
        ofs << "queso_" << m_prefix << chainId << "_" << "misfitVarianceChain = [";
        for (unsigned int i = 0; i < m_misfitVarianceChain.size(); ++i) {
      ...
      ofs << "queso_" << m_prefix << chainId << "_" << "m2lLikelihoodChain = zeros(" << m_m2lLikelihoodChain.size()
          << ","                                                                     << m_m2lLikelihoodChain[0]->size()
          << ");"
          << std::endl;
      ofs << "queso_" << m_prefix << chainId << "_" << "m2lLikelihoodChain = [";
      for (unsigned int i = 0; i < m_m2lLikelihoodChain.size(); ++i) {
      ...
      ofs << "queso_" << m_prefix << chainId << "_" << "alphaQuotients = zeros(" << m_alphaQuotients.size()
          << ","                                                                 << 1
          << ");"
          << std::endl;
      ofs << "queso_" << m_prefix << chainId << "_" << "alphaQuotients = [";
  ...
  ofs << "queso_" << m_prefix << chainId << "_" << "paramNames = {";
  ...
  if (mahalanobisMatrix != NULL) {
    ...
    ofs << "queso_" << m_prefix << chainId << "_" << "d = [";
  ...
  ofs << "queso_" << m_prefix << chainId << "_" << "priorMeanValues = ["
      << m_paramSpace.priorMuValues()
      << "];\n";

  // Write prior sigma values
  ofs << "queso_" << m_prefix << chainId << "_" << "priorSigmaValues = ["
      << m_paramSpace.priorSigmaValues()
      << "];\n";

#if 0
  ofs << "queso_" << m_prefix << chainId << "_" << "results.prior = [queso_priorMeanValues',queso_priorSigmaValues'];\n";
#endif

  // Write param lower bounds
  ofs << "queso_" << m_prefix << chainId << "_" << "minValues = ["
      << m_paramSpace.minValues()
      << "];\n";

  // Write param upper bounds
  ofs << "queso_" << m_prefix << chainId << "_" << "maxValues = ["
      << m_paramSpace.maxValues()
      << "];\n";

#if 0
  ofs << "queso_" << m_prefix << chainId << "_" << "results.limits = [queso_low',queso_upp'];\n";

  // Write out data for mcmcpred.m
  ofs << "queso_" << m_prefix << chainId << "_" << "results.parind = ["; // FIXME
  for (unsigned int i = 0; i < m_paramSpace.dim(); ++i) {
    ofs << i+1
        << std::endl;
  }
  ...
  ofs << "queso_" << m_prefix << chainId << "_" << "results.local = [\n"; // FIXME
  for (unsigned int i = 0; i < m_paramSpace.dim(); ++i) {
    ofs << " 0";
    //<< std::endl;
  }
  ...
  if (m_useChain2) {
    ...
    bool savedVectorPrintState = chain1[chain1.size()-1]->getPrintHorizontally();
    chain1[chain1.size()-1]->setPrintHorizontally(false);
    ofs << "queso_" << m_prefix << chainId << "_" << "results.theta = ["
        << *(chain1[chain1.size()-1])
        << "];\n";
    chain1[chain1.size()-1]->setPrintHorizontally(savedVectorPrintState);
  }
  
  ofs << "queso_" << m_prefix << chainId << "_" << "results.nbatch = 1.;\n"; // FIXME

  if (mahalanobisMatrix != NULL) {
    // Write covMatrix
    ofs << "queso_" << m_prefix << chainId << "_" << "mahalanobisMatrix = ["
        << *mahalanobisMatrix
        << "];\n";
  }
#endif
  ...
  if (m_useChain2) {
    ofs << "queso_" << m_prefix << chainId << "_" << "rejected = "  << (double) m_numRejections/(double) (chain2.sequenceSize()-1)
        << ";\n"
        << std::endl;
  }
  else {
    ofs << "queso_" << m_prefix << chainId << "_" << "rejected = "  << (double) m_numRejections/(double) (chain1.size()-1)
        << ";\n"
        << std::endl;
  }
  ...
  if (m_useChain2) {
    ...
    ofs << "queso_" << m_prefix << chainId << "_" << "outbounds = " << (double) m_numOutOfBounds/(double) chain1.size()
        << ";\n"
        << std::endl;
  }
...
uqDRAM_MarkovChainGeneratorClass<V,M>::print(std::ostream& os) const
{
  os <<         m_option_chain_type   << " = " << m_chainType
     << "\n" << m_option_chain_number << " = " << m_numberOfChains
