  ...
  bool     applyMahalanobisInvert)
{
  ...
  if (m_generateWhiteNoise) {
    for (unsigned int chainId = 0; chainId < m_sizesOfChains.size(); ++chainId) {
      ...
      //****************************************************
      iRC = generateWhiteNoise2(chainId);
    ...
    for (unsigned int chainId = 0; chainId < m_sizesOfChains.size(); ++chainId) {
      ...
      computeStatistics2(m_uniqueChain2);
#endif
      //****************************************************
      // Write chain2 out
      //****************************************************
      iRC = writeChain2(chainId,
                        mahalanobisMatrix,
                        applyMahalanobisInvert);
      UQ_FATAL_RC_MACRO(iRC,
                        m_env.rank(),
...
template <class V, class M>
int
uqDRAM_MarkovChainGeneratorClass<V,M>::generateWhiteNoise2(unsigned int chainId)
{
  if (m_env.rank() == 0) {
    std::cout << "Generating white noise for chain2 of id " << chainId
  ...
  m_chain2.resizeSequence(m_sizesOfChains[chainId]); 
  ...
  bool         applyMahalanobisInvert)
{
  ...
  //****************************************************
  m_chain2.resizeSequence(m_sizesOfChains[chainId]); 
  if (m_generateUniqueChain) m_uniqueChain2.resizeSequence(m_sizesOfChains[chainId]); 
  ...
  if (m_generateUniqueChain) m_uniqueChain2.setPositionValues(m_uniqueChain2Pos++,currentPosition.paramValues());
  if (m_generateExtraChains) {
    if (m_likelihoodObjComputesMisfits) {
  ...
  for (unsigned int positionId = 1; positionId < m_chain2.sequenceSize(); ++positionId) {
    ...
    if (accept) {
      ...
      if (m_generateUniqueChain) m_uniqueChain2.setPositionValues(m_uniqueChain2Pos++,currentCandidate.paramValues());
      if (m_generateExtraChains) {
        if (m_likelihoodObjComputesMisfits) {
  ...
  if (m_env.rank() == 0) {
    if (m_generateUniqueChain) {
      std::cout << "Finished generating the chain2 of id " << chainId
                << ", with "                               << m_uniqueChain2Pos
  ...
  const uqArrayOfSequencesClass<V>& chain2)
{
  ...
  if ((m_computePSDs                ) &&
    ...
    if (m_env.rank() == 0) {
      for (unsigned int initialPosId = 0; initialPosId < m_initialPosForPSD.size(); initialPosId++) {
        double sizeForPSD = chain2.sequenceSize() - m_initialPosForPSD[initialPosId];
  ...
  if ((m_computeGewekeCoefs            ) &&
    ...
    chain2.geweke(m_initialPosForGeweke,
                  m_ratioNaForGeweke,
                  m_ratioNbForGeweke,
                  vectorOfGeweke);
  ...
  if ((m_computeCorrelations          ) &&
    ...
    chain2.autoCorrelations(m_initialPosForCorrs,
                            m_lagsForCorrs,
                            _2dArrayOfAutoCorrs);
  ...
  bool         applyMahalanobisInvert) const
{
  ...
  if (m_namesOfOutputFiles[chainId] == ".") {
    if (m_env.rank() == 0) {
      std::cout << "No info written out for chain2 of id " << chainId
                << std::endl;
    }
    return iRC;
  }
  ...
  if (m_env.rank() == 0) {
    std::cout << "Writing out info about the chain2 of id " << chainId
  ...
  if (m_env.rank() == 0) {
    std::cout << "Finished writing out info about the chain2 of id " << chainId
              << std::endl;
  }
