...
struct
uqPhysics1LikelihoodInfoStruct
{
  uqPhysics1LikelihoodInfoStruct(const uqVectorSpaceClass<P_V,P_M>& paramSpace,
...
template<class P_V,class P_M>
uqPhysics1LikelihoodInfoStruct<P_V,P_M>::uqPhysics1LikelihoodInfoStruct(
  ...
  m_paramSpace(paramSpace)
{
  if ((paramSpace.env().verbosity() >= 30) && (paramSpace.env().rank() == 0)) {
    std::cout << "Entering uqPhysics1LikelihoodInfoStruct<P_V,P_M>::constructor()"
              << inpName << "'\n"
              << std::endl;
  }
  ...
  if (inp == NULL) {
    ...
    UQ_FATAL_TEST_MACRO((inp == NULL),
                        paramSpace.env().rank(),
                        "uqPhysics1LikelihoodInfoStruct<P_V,P_M>::constructor()",
  ...
  if ((paramSpace.env().verbosity() >= 30) && (paramSpace.env().rank() == 0)) {
    std::cout << "Leaving uqPhysics1LikelihoodInfoStruct<P_V,P_M>::constructor()"
              << inpName << "'\n"
              << std::endl;
  }
}
  ...
  P_V*        hessianEffect)
{
  ...
  if ((paramValues.env().verbosity() >= 30) && (paramValues.env().rank() == 0)) {
    std::cout << "Entering uqPhysics1LikelihoodRoutine()..." << std::endl;
  }

  if ((paramValues.env().verbosity() >= 10) && (paramValues.env().rank() == 0)) {
    std::cout << "In uqPhysics1LikelihoodRoutine()"
              << ", A = " << paramValues[0]
              << ", E = " << paramValues[1]
              << std::endl;
  }
  ...
  // Loop on scenarios
  const std::vector<uqPhysics1LikelihoodInfoStruct<P_V,P_M> *>& vecInfo = *((const std::vector<uqPhysics1LikelihoodInfoStruct<P_V,P_M> *> *) functionDataPtr);
  ...
  if ((paramValues.env().verbosity() >= 30) && (paramValues.env().rank() == 0)) {
    std::cout << "Leaving uqPhysics1LikelihoodRoutine()..." << std::endl;
  }

  return resultValue;
}

#endif // __UQ_PHYSICS_1_LIKELIHOOD_H__
