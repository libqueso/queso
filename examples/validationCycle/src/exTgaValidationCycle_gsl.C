 ...
 *
 * Example (1) focuses on the templated class uqStatisticalInverseProblem<P_V,P_M>.
 * Example (2) focuses on the templated class uqStatisticalForwardProblem<P_V,P_M,Q_V,Q_M>.
 * Example (3) focuses on both classes, since it uses the solution of the inverse problem as input to the forward problem.
 *
 * This example with TGA uses both statistical problem templated classes, twice each in fact, but it also:
 * - uses the templated class uqValidationCycle<P_V,P_M,Q_V,Q_M>,
...
#else
  QUESO::FullEnvironment* env = new QUESO::FullEnvironment(0,argv[1],"",envOptionsValues);
#endif
#else
...
#else
  QUESO::FullEnvironment* env = new QUESO::FullEnvironment(0,"","",envOptionsValues);
#endif
#endif
