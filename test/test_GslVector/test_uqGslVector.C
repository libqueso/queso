...
int checkLinearSpacing(const uqGslVectorClass &v, double d1, double d2) {
  unsigned int i;
  double alpha, elt;
...
int main(int argc, char **argv) {
  ...
  uqGslVectorClass v1(param_space->zeroVector());
  v1.cwSet(2.0);

  uqGslVectorClass *v2 = new uqGslVectorClass(*env, d1, d2, v1.map());
  if (checkLinearSpacing(*v2, d1, d2) == 1) {
    std::cerr << "Linear spacing test 1 failed" << std::endl;
  }
  delete v2;

  v2 = new uqGslVectorClass(v1, d1, d2);
  if (checkLinearSpacing(*v2, d1, d2) == 1) {
    std::cerr << "Linear spacing test 2 failed" << std::endl;
  }

  uqGslVectorClass v3(*v2);
  *v2 /= v1;
  ...
  uqVectorSpaceClass<uqGslVectorClass, uqGslMatrixClass> big_param_space(*env,
      "", 6, NULL);
  uqGslVectorClass v4(big_param_space.zeroVector());
  ...
  std::vector<const uqGslVectorClass*> vecs;
  vecs.push_back(&v1);
  vecs.push_back(&v3);
  uqGslVectorClass v5(big_param_space.zeroVector());
  ...
  uqGslVectorClass ones(v1, 1.0, 1.0);
  if (!(ones == (v1 / v3))) {
