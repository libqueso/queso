...
# DESCRIPTION
#
#   Try to disable C compiler optimization(s). Macro based in part on
...
if test "$ac_test_CFLAGS" != "set"; then
  ...
  case $ax_cv_c_compiler_vendor in
    gnu) 
    	 CFLAGS="$CFLAGS -fno-unsafe-math-optimizations -Dgnu_compiler"
         ;;

    intel) 
    	 CFLAGS="$CFLAGS -fp-model precise -Dintel_compiler"
	 ;;

    portland)
    	 CFLAGS="$CFLAGS -Kieee -Mnofpapprox -Dportland_compiler"
	 ;;	

  esac	 
  
  AC_MSG_NOTICE([disabling C compiler optimizations ($CFLAGS)])
    ...
    echo "$delim user-provided CFLAGS ($CFLAGS)."    
