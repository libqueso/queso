...
SfpOptionsValues::getOptionValues()
{
  ...
  if ((*m_optionsMap).count(m_option_computeCovariances)) {
    m_computeCovariances = ((const po::variable_value&) (*m_optionsMap)[m_option_computeCovariances]).as<bool>();
  }

  if ((*m_optionsMap).count(m_option_computeCorrelations)) {
    m_computeCorrelations = ((const po::variable_value&) (*m_optionsMap)[m_option_computeCorrelations]).as<bool>();
  }
