...
class uqGcmJointTildeInfoClass
{
public:
  uqGcmJointTildeInfoClass(const uqGpmsaComputerModelOptionsClass&                     gcmOptionsObj,
                           const uqGcmExperimentInfoClass<S_V,S_M,D_V,D_M,P_V,P_M>&    e,
                           const uqGcmJointInfoClass<S_V,S_M,D_V,D_M,P_V,P_M,Q_V,Q_M>& jj);
 ~uqGcmJointTildeInfoClass();

  const uqBaseEnvironmentClass&     m_env;
        D_M                         m_Bmat_tilde;
        unsigned int                m_Bmat_tilde_rank;
        uqVectorSpaceClass<D_V,D_M> m_vu_tilde_space;
        D_M                         m_Lbmat;
...
template <class S_V,class S_M,class D_V,class D_M,class P_V,class P_M,class Q_V,class Q_M>
  uqGcmJointTildeInfoClass<S_V,S_M,D_V,D_M,P_V,P_M,Q_V,Q_M>::uqGcmJointTildeInfoClass(
  ...
  tmpSet.insert(m_env.subId());
    ...
    if (jj.m_Bmat_with_permut->numRowsGlobal() >= jj.m_Bmat_with_permut->numCols()) {
      ...
      if (m_env.subDisplayFile()) {
        *m_env.subDisplayFile() << "In uqGcmJointTildeInfoClass<S_V,S_M,D_V,D_M,P_V,P_M,Q_V,Q_M>::constructor()"
    ...
    UQ_FATAL_TEST_MACRO(m_Bmat_tilde_rank != std::min(m_Bmat_tilde.numRowsGlobal(),m_Bmat_tilde.numCols()),
                        m_env.worldRank(),
                        "uqGcmJointTildeInfoClass<S_V,S_M,D_V,D_M,P_V,P_M,Q_V,Q_M>::constructor()",
